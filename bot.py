...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    testo = (
        ...
        "🎯 Scrivi `/gioca_con @username` per invitare un amico che ha già avviato il bot\n"
        ...
        "Il gioco è in aggiornamento, potrebbe subire dei malfunzionamenti"
    )
...
async def messaggio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Se stiamo aspettando l'età dell'utente
    if context.user_data.get("attesa_eta"):
        if not update.message.text:
            await update.message.reply_text("⛔ Scrivimi la tua età in formato numerico, non mandare foto o altro.")
            return
        ...
        return  # Fine gestione età, non proseguiamo con la chat

    # Se non è in chat anonima
    ...
    # 7️⃣ Promemoria per la prossima domanda ogni 8 messaggi
...
async def setta_comandi(bot):
    comandi = [
        ...
        BotCommand("gioca_con", "Invita un amico a giocare"),
        BotCommand("accetta", "Accetta invito di un amico"),
